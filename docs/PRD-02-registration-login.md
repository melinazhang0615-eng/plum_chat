...
本文的产品决策**已冻结，正文保持原样**。下面说明当前实现到哪一步，供测试和验收对照。

主链路已跑通：匿名浏览 → Welcome（18+ 与称呼）→ 游客免费试聊（服务端额度）→ 高意图点
登录墙 → Email OTP 或 Google 注册 → 游客数据合并 → 首登 1000 金币幂等发放。

两项第 3 节已冻结决策**尚未实现**：

- **邀请码账号绑定正式身份**（第 3 节「邀请码」行）：绑定接口尚未开发，邀请码账号目前只能
  继续用邀请码登录，还不能升级为 Email/Google 身份。邀请码入口在此之前不能下线。
- **Apple 登录**：本就属于第 2.2 节非目标，V1 默认关闭。当前仅保留能力位与 UI 位置，
  没有 Apple adapter，也未做任何验证；待原生 App 上架计划与 Apple 配置就绪后再实现。

技术侧的逐项对照见 [TECH-09 §0 实现状态](./TECH-09-registration-login.md)。
